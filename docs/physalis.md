...
- Stockage : `ClientEmailConfig` (activation + compte Physalis Email partagé par tenant) et `ProjectEmailConfig` (domaine + **clé API chiffrée** + DNS + `defaultSender`). Les variables runtime (`PHYSALIS_EMAIL_API_KEY`, `PHYSALIS_EMAIL_DOMAIN`, `PHYSALIS_EMAIL_URL`, `PHYSALIS_EMAIL_FROM`) sont injectées dans le `.env` de chaque environnement **au déploiement** ([app/api/deploy/route.ts](../app/api/deploy/route.ts)), jamais stockées en `Secret` éditable. *(Renommées depuis `PINK_FLOYD_*` en 2026-07 — rename sec, aucun client n'avait configuré le service à ce stade.)*
- **Expéditeur principal (`PHYSALIS_EMAIL_FROM`, 2026-07-17).** L'onglet **Expéditeurs** est une **liste blanche** côté relais : il autorise des adresses, il n'en choisit pas. Le relais exige donc un `from` de l'appelant, et le client devait poser un `Secret` `EMAIL_FROM` à la main. Désormais un expéditeur est marqué **principal** (`ProjectEmailConfig.defaultSender`, migration `20260717120000_project_email_default_sender`) et son adresse est injectée en `PHYSALIS_EMAIL_FROM`. **Le premier expéditeur créé le devient d'office** ; supprimer le principal remet le champ à `null` (on ne promeut personne d'office : ça changerait l'adresse d'envoi de l'app dans son dos — mieux vaut un échec bruyant).
  - **Seule l'adresse est stockée côté Physalis.** Le **nom d'affichage** vit côté relais (sur le sender) et y est appliqué à l'envoi (`"Nom" <adresse>`, [send.ts](../../physalis-email/src/routes/send.ts)) → renommer un expéditeur ne demande **pas** de redeploy ; changer le principal, si.
  - **L'injection ne parle jamais au relais** : `defaultSender` est lu sur la ligne `ProjectEmailConfig` déjà chargée par `/api/deploy`. Interroger le relais y mettrait son Redis sur le chemin critique de **tous** les déploiements.
  - Préfixée `PHYSALIS_EMAIL_*` et non `EMAIL_FROM` : trop générique pour être posée d'autorité dans le `.env` d'un client qui peut déjà s'en servir pour un autre fournisseur. Un `Secret` explicite du projet reste prioritaire sur l'injection (`if (!(k in secrets))`).
...
> **Rename `pink-floyd` → `physalis-email` (2026-07) — TERMINÉ.** Les vars serveur portent les noms `EMAIL_PHYSALIS_URL`/`EMAIL_PHYSALIS_API_KEY` et, côté module email par projet, `PHYSALIS_EMAIL_SERVICE_URL`/`PHYSALIS_EMAIL_SERVICE_KEY`/`PHYSALIS_EMAIL_ENABLED`/`PHYSALIS_EMAIL_ALLOWED_EMAILS`. Le repli temporaire sur les anciens noms `PINK_FLOYD_*`, qui a permis de déployer le code renommé avant la bascule de l'env prod, a été retiré une fois celle-ci faite : **plus aucun ancien nom n'est lu**. Le service est joignable sur `https://email.physalis.cloud` (ex-`pink-floyd.argoweb.fr`).
>
